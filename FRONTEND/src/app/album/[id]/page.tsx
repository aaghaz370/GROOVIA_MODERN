    ...
    const fetchAlbumDetails = async () => {
        try {
            ...
            if (idStr.startsWith('MPREb') || idStr.startsWith('OLAK5')) {
                try {
                    const res = await fetch(`${process.env.NEXT_PUBLIC_YT_API_URL || 'http://localhost:8005'}/album?browseId=${params.id}`);
                    ...
                    const normalizedAlbum: AlbumData = {
                        ...
                        songs: ytData.tracks?.map((t: any) => {
                            ...
                            return {
                                ...
                                downloadUrl: [{ quality: '320kbps', url: `http://localhost:8000/api/player/stream?videoId=${t.videoId}` }],
                                url: `http://localhost:8000/api/player/stream?videoId=${t.videoId}`
                            };
                        }) || []
                    };
